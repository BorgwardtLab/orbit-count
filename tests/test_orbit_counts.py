...
def _baseline_implementation(
    ...
    try:
        ...
        subprocess.check_output(
            [orca_executable, orbit_type, str(graphlet_size), in_fname, out_fname]
        )
        with open(out_fname, "r") as out_tmp:
            counts = [
                [int(item) for item in line.split(" ")] for line in out_tmp.readlines()
            ]
    finally:
        os.unlink(in_fname)
...
@pytest.mark.parametrize(
    "n,p", [(100, 0.05), (100, 0.1), (16, 0.1), (16, 0.2), (32, 0.02)]
)
def test_erdos_renyi(orca_executable, orbit_type, n, p, graphlet_size, seed):
    ...
    if orbit_type == "node":
        counts = orbit_count.node_orbit_counts(
            graph, graphlet_size=graphlet_size, node_list=node_list
        )
...
def test_node_list(n, p, graphlet_size, seed):
    ...
    res1 = orbit_count.node_orbit_counts(
        graph, graphlet_size=graphlet_size, node_list=node_list
    )
    res2 = orbit_count.node_orbit_counts(
        graph, graphlet_size=graphlet_size, node_list=node_list_perm
    )
...
def test_edge_list(n, p, graphlet_size, seed):
    ...
    res1 = orbit_count.edge_orbit_counts(
        graph, graphlet_size=graphlet_size, edge_list=edge_list
    )
    res2 = orbit_count.edge_orbit_counts(
        graph, graphlet_size=graphlet_size, edge_list=edge_list_perm
    )
...
def test_memory_leak(orbit_type, graphlet_size):
    ...
    for _ in range(iterations):
        if orbit_type == "node":
            _ = orbit_count.node_orbit_counts(graph, graphlet_size=graphlet_size)
        else:
            _ = orbit_count.edge_orbit_counts(graph, graphlet_size=graphlet_size)
    ...
    assert (
        memory_increase < acceptable_increase
