...
def node_orbit_counts(graph: nx.Graph, graphlet_size: int = 4, node_list: Optional[List] = None) -> np.ndarray:
    return motif_counts("node", graphlet_size, graph.number_of_nodes(), _edge_list_reindexed(graph, node_list=node_list))


def edge_orbit_counts(graph: nx.Graph, graphlet_size: int = 4, edge_list: Optional[List] = None) -> np.ndarray:
    return motif_counts("edge", graphlet_size, graph.number_of_nodes(), _edge_list_reindexed(graph, edge_list=edge_list))
