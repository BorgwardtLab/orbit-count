...
private:
    ...
    PII** inc;              // incidence lists
    int* adj_matrix;        // compressed adjacency matrix
    std::vector<PAIR> edges; // stored edges
	bool use_matrix;
...
public:
	GraphData(int num_nodes, const std::vector<std::pair<int, int>>& edge_index) 
        : n(num_nodes), deg(nullptr), adj(nullptr), inc(nullptr), 
          adj_matrix(nullptr), use_matrix(true) {
	...
	std::vector<std::vector<int>> count4() {
		int frac,frac_prev;
		int m = edges.size();
		...
		std::vector<std::vector<int>> result = generate_node_orbit_matrix(4, orbit);
	...
	std::vector<std::vector<int>> ecount4() {
		int frac,frac_prev;
		int m = edges.size();

		int64 **eorbit = (int64**)malloc(m*sizeof(int64*));
		...
		std::vector<std::vector<int>> result = generate_edge_orbit_matrix(4, eorbit);
	...
	std::vector<std::vector<int>> count5() {
		...
		std::vector<std::vector<int>> result = generate_node_orbit_matrix(5, orbit);
	...
	std::vector<std::vector<int>> ecount5() {
		unordered_map<PAIR, int, hash_PAIR> common2;
		unordered_map<TRIPLE, int, hash_TRIPLE> common3;
		unordered_map<PAIR, int, hash_PAIR>::iterator common2_it;
		unordered_map<TRIPLE, int, hash_TRIPLE>::iterator common3_it;
		...
		std::vector<std::vector<int>> result = generate_edge_orbit_matrix(5, eorbit);
...
std::vector<std::vector<int>> motif_counts(std::string orbit_type, int graphlet_size, int num_nodes, const std::vector<std::pair<int, int>>& edge_index) {
	GraphData graph(num_nodes, edge_index);
	if(orbit_type == "node"){
		if(graphlet_size == 4) return graph.count4();
		if (graphlet_size == 5) return graph.count5();
	}
	if(orbit_type == "edge"){
		if(graphlet_size == 4) return graph.ecount4();
		if (graphlet_size == 5) return graph.ecount5();
	}
